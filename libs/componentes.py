...
@st.cache_data
def get_image_base64(name_img):

    dir_path = os.path.dirname(os.path.realpath(__file__))
    image_path = os.path.join(dir_path, name_img)
...
def chatbot_component(df):
    ...
    Proposito:
            Através dos sinais oriundos de usinas hidroeleticas é gerado um prompt para que uma IA possa responder perguntas sobre o estado da usina.
    Estados:
            1 - Estado
    ...
    # 2 - Mensagens - onde as mensagens do chatbot serão exibidas
    messages = st.empty()

    # 3 - Imagem do chatbot
    image_base64 = get_image_base64("img.webp")

    # 4 - Inicializa a lista de mensagens
    if "chatbot_messages" not in st.session_state:
        st.session_state.chatbot_messages = []

    # 5 - Inicializa o chatbot criando o primeiro prompt
    if len(st.session_state.chatbot_messages) == 0:
        ...
        # executa o prompt inicial
        resp = 'Olá, tudo bem? Como posso te ajudar?'
        message = resp

        # adiciona a mensagem ao state do chatbot
